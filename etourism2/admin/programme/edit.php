...
if (
    ...
    // Check if the programme exists based on `name`
    ...
    if ($stmtCheck->rowCount() > 0) {
        ...
        if ($uploadOk == 1) {
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                ...
                $stmtUpdate = $con->prepare("UPDATE programme SET description = ?, start_date = ?, end_date = ?, price = ?, image_path = ? WHERE name = ?");
        ...
        // If the programme is not found, return a fail state
