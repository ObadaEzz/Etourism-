...
class DeleteProgrammePage extends StatefulWidget {
  @override
  _DeleteProgrammePageState createState() => _DeleteProgrammePageState();
}

class _DeleteProgrammePageState extends State<DeleteProgrammePage> {
  ...
  // دالة لحذف البرنامج
  Future<void> _deleteProgramme() async {
    if (_formKey.currentState!.validate()) {
      ...
      var response =
          await postRequest(programmeDelete, data); // إرسال طلب الحذف
      ...
      if (response != null && response['status'] == 'success') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Programme not found. No action taken.'),
          backgroundColor: Colors.red,
        ));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(
              'Failed to delete programme: ${response?['message'] ?? 'Unknown error'}'),
          backgroundColor: Colors.red,
        ));
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Delete Programme'), // تغيير العنوان إلى "حذف برنامج"
          ...
          child: ListView(
            children: [
              _buildTextFormField(
                ...
                labelText: 'Programme Name', // تغيير العنوان إلى معرف البرنامج
                validator: (value) {
                  if (value == null || value.isEmpty) {
                    return 'Please enter programme name'; // رسالة الخطأ إذا كان المدخل فارغًا
                  }
                  return null;
                },
              ),
              ...
              ElevatedButton(
                onPressed: isLoading ? null : _deleteProgramme,
                ...
                child: isLoading
                    ...
                    : const Text(
                        'Delete Programme'), // تغيير النص إلى "حذف برنامج"
              ),
            ],
          ),
        ),
